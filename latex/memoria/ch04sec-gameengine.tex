...
Se han realizado dos casos de prueba:
\begin{itemize}
\item Ejecutando primero el método territoryUnderAttack. Para poder ejecutar el método resolver ataque se necesita que primero la variable mCurrentAttack tenga un valor no nulo, por este motivo ejecutamos antes el método territoryUnderAttack y luego el método resolver ataque no da error.
\item Sin ejecutar primero el método territoryUnderAttack. En este caso no se le da un valor a la varible mCurrentAttack y como es null falla la ejecución de resolver ataque.
\end{itemize}
