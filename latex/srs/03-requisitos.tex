...
\item La ventana de la partida muestra un mensaje indicando: "El jugador ... se 
ha conectado a la partida ...".
\item Si se está jugando esta partida, el servidor enviará una actualización de 
la partida al gestor de partidas del usuario con la localización y los recursos 
del nuevo jugador.
\item El gestor de partidas envia la información de la partida a la ventana de 
la partida.
\item La ventana de la partida se actualiza la posición que ocupa el nuevo 
jugador, los soldados que tiene y sus gallifantes.
\end{enumerate} \\

\textbf{Poscondición} & \\ \\

\textbf{Escenario alternativo} & El jugador está conectado en el sistema jugando 
a otra partida.\begin{enumerate}
\item El servidor envia al gestor de partidas el nombre del nuevo jugador y la 
partida a la que se quiere conectar.
\item El gestor de usuarios pasa esta información a la ventana de la partida.
\item La ventana de la partida muestra un mensaje indicando: "El jugador ... se 
...
partida \\
% Destino: mismo jugador, sin asignar y aceptar ataque.
\textbf{Escenario general} & \begin{enumerate}
\item El usuario selecciona el origen, el destino del movimiento y el número de
tropas a mover o atacar al territorio destino en la ventana de la partida.
\item La ventana de la partida comprueba que el movimiento es legal (casilla 
adyacente).
...
% En aceptar ataque se preguna al otro jugador con el caso de uso Responder a 
% acción enemiga
\item El servidor manda la actualización del mapa con el movimiento o ataque 
realizado al gestor de partidas.
\item El gestor de partidas pasa la información a la ventana de la partida.
\item La ventana de la partida actualiza el mapa de la partida.
\end{enumerate} \\
...
% Se ofrecen soldados o gallifantes, da igual que se acepten o no:
% si recibes actualización de la oferta.
% no recibes actualización del resultado del ataque.
& Se realiza una oferta para no atacar.\begin{enumerate}
...
\item La ventana de la partida envía la información al gestor de partidas.
\item El gestor de partidas envía la información al servidor através del proxy.
\item El servidor devuelve la oferta para que no ataque al gestor de partidas.
\item El gestor de partidas pasa a la ventana de partidas la oferta.
\item La ventana de la partida pregunta si se acepta la oferta.
\item La ventana envía la respuesta a la oferta al gestor de partidas.
\item El gestor de partidas envía la respuesta a la oferta al servidor através 
del proxy.
\item El servidor devuelve la actualización con la resolución del movimiento 
realizado al gestor de partidas.
\item El gestor de partidas pasa la información a la ventana de la partida.
\item La ventana de la partida actualiza la partida con el resultado del 
movimiento. 
\end{enumerate} \\
...
\item El servidor devuelve la actualización del mapa con el resultado de la 
acción al gestor de partidas.
\item El gestor de partidas pasa la información a la ventana de la partida.
\item La ventana de la partida actualiza el mapa de la partida con el resultado 
la acción.
